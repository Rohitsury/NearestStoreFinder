...
function OtpModal({ setModal, userId, email }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/storemodule/verifyotp",
        {
          method: "POST",
          headers: {
  ...
  async function Resendotp(e) {
    ...
    try {
      const response = await fetch(
        "http://localhost:5000/storemodule/resentOtp",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <>
      <form className="loginForm w-75 p-lg-5 p-3" onSubmit={handleSubmit}>
        ...
        <div className="d-flex">
          <button type="submit" class=" loginButton py-2 me-3">
            Verify
          </button>
          <button onClick={Resendotp} class=" loginButton py-2 me-3">
            Resend OTP
          </button>
        </div>
      </form>
